...
class GameScreen(Screen):
    ...
    def _on_text(self, instance, value):
        ...
        if verse_changed or game_just_finished:
            Clock.schedule_once(lambda dt: self._clear_input(), 0)
...
class MPGameScreen(Screen):
    ...
    def _on_text(self, player_idx, instance, value):
        ...
        if result is not None and result != value:
            Clock.schedule_once(
                lambda dt: setattr(self.inputs[player_idx], 'text', result), 0)
            return
    ...
    def _clear_mp_input(self, idx):
        """Clear multiplayer input for player idx and refresh label."""
        self.inputs[idx].text = ""
        self._refresh_verse_label(idx, self.games[idx])

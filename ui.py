...
class MultiTrackWidget(Widget):
    """Shows all cars on a single shared track for live position comparison."""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.cars = []   # [(name, color, progress), ...]
        self.bind(pos=self._draw, size=self._draw)
    ...
    def _draw(self, *_):
        ...
        with self.canvas:
            ...
            for x in range(0, int(self.width), step):
                Line(points=[self.x + x, y_mid,
                              self.x + x + dp(10), y_mid], width=1)

            n = len(self.cars)
            if n == 0:
                return
            lane_h = self.height / n
            for i, (name, color, progress) in enumerate(self.cars):
                cw   = dp(22)
                ch   = lane_h * 0.55
                cx   = self.x + dp(3) + progress * (self.width - cw - dp(6))
                cy   = self.y + i * lane_h + (lane_h - ch) / 2
                Color(*color)
                Rectangle(pos=(cx, cy), size=(cw, ch))
                # Name tag
                Color(*TEXT_COLOR)
...
class GameScreen(Screen):
    ...
    def setup_game(self, mode, ai_count=0, ai_difficulty="rookie"):
        ...
        # Multi-track (AI position overview)
        if self.ai_opponents:
            self.multi_track = MultiTrackWidget(size_hint_y=None, height=dp(55))
    ...
    def _on_text(self, instance, value):
        ...
        # Verse just completed → reset input & update AI target length
        if self.game.typed_text == "" and value != "":
    ...
    def _update(self, dt):
        ...
        if self.multi_track:
            cars = [("You", [0.2, 0.7, 0.3, 1], self.game.car_position)]
            for ai in self.ai_opponents:
                cars.append((ai.name, list(ai.color), ai.get_progress()))
...
class MPGameScreen(Screen):
    ...
    def _on_text(self, player_idx, instance, value):
        ...
        # Verse was completed → clear input
        if game.typed_text == "" and value != "":
